    ...
    >
      <div className="container mx-auto px-4">
        ...
        <div className="relative mx-auto max-w-4xl"> {/* Container for the timeline */}
          {/* Vertical line of the timeline - visible only on medium screens and up */}
          ...
          {educationData.map((edu, index) => (
            <motion.div
              key={edu.id}
              // Stack vertically on small screens, then alternate sides on medium and up
              className={`flex flex-col md:flex-row items-center w-full my-8 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}
              initial={{ opacity: 0, x: index % 2 === 0 ? -100 : 100 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ delay: 0.1 * index, duration: 0.7 }}
            >
              {/* Card content - full width on small screens, half width on medium and up */}
              ...
              {/* Timeline dot - visible only on medium screens and up */}
              <div className="hidden md:flex w-0 md:w-1/12 justify-center z-20">
              ...
              {/* Empty div to balance layout - hidden on small screens */}
